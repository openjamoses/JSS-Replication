...
data_file = open(path_output + 'repos_commits_loc_interval_metrics.csv', mode='w', newline='', encoding='utf-8')
data_writer = csv.writer(data_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
data_writer.writerow(["categories",'repos','repo_id', "interval","commits", 'cloc'])
...
for cat in categories:
    if not cat in ignore_cat:
        ...
        for repo_id in range(len(Repos_Name2)):
            if Type[repo_id] == cat:
                repo_id_list.append(Repos_Name2[repo_id])
        for repo_id in range(len(Repos_Name2)):
            if Type[repo_id] == cat:
                num += 2
                ...
                while True:
                    ...
                    if commit_obj != None:
                        ...
                        for obj in commit_obj:
                            ...
                            if ct == len(get_tokens()):
                                ct = 0
                            loc, ct = GitHubMeta(Repos_Name[index], ct).get_hash_cloc_(shaa)
                                ...
                                if compare_dates(d_to, date[:10]) and compare_dates(date[:10], d_from):
                                    ...
                                    if key in dict_dates_vals2.keys():
                                        if key in dict_dates_vals.keys():
                                            ...
                                            dict_dates_loc_vals2[key].append(dict_dates_loc_vals.get(key))
                                        else:
                                            dict_dates_vals2[key] = [0]
                                            dict_dates_loc_vals2[key] = [0]
                                    else:
                                        if key in dict_dates_vals.keys():
                                            dict_dates_vals2[key] = [dict_dates_vals.get(key)]
                                            dict_dates_loc_vals2[key] = [dict_dates_loc_vals.get(key)]
                                        else:
                                            dict_dates_vals2[key] = [0]
                                            dict_dates_loc_vals2[key] = [0]
                        ...
                        break
                for key, val in dict_dates_vals.items():
                    data_writer.writerow([cat,  Repos_Name[index], repo_id_list.index(Repos_Name[index]), key, val, dict_dates_loc_vals.get(key)])
