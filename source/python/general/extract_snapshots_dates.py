...
def main():
    ...
    output_path = '/Volumes/Cisco/Fall2021/quantum-revision/Analysis/general/'
    file = 'MLRepos_Quantum_non_quantum.xlsx'
    xl1 = pd.ExcelFile(path + file)
    df_topic = xl1.parse('quant_non_quantum')
    #df_topic = pd.read_csv("{}{}".format(path, file))
    # Convert to list
    Repos_Name = df_topic.Repos_Name.values.tolist()
    Language = df_topic.Language.values.tolist()
    ...
    for index in range(len(Repos_Name)):
        ...
        if os.path.exists(repo_target_path_root+'/'+str(Repos_Name[index].split('/')[1])+'_'+str(index)):
            print( '      --- snapshots exist')
            #if cloned == 0:
            ...
            repo_path = repo_target_path_root+'/'+str(Repos_Name[index].split('/')[1])+'_'+str(index) #repo_target_path_root + "/" + str(file_name[index]).replace('_snapshots', '')
